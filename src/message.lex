...
# Inverse of `artifact_to_json`. The durable task store round-trips a
# Task through JSON, so a stored Task's artifacts come back through
# here. Missing `name` / `index` default rather than error — A2A
# treats artifact metadata as advisory.
fn parse_artifact(j :: jv.Json) -> Result[Artifact, Str] {
  let name := match jv.get_field(j, "name") {
    Some(v) => match jv.as_str(v) { Some(s) => s, None => "" },
    None    => "",
  }
  let index := match jv.get_field(j, "index") {
    Some(v) => match jv.as_int(v) { Some(n) => n, None => 0 },
    None    => 0,
  }
  match jv.get_field(j, "parts") {
    None     => Err("artifact missing `parts`"),
    Some(pj) => match jv.as_list(pj) {
      None        => Err("artifact `parts` must be an array"),
      Some(items) => match parse_parts(items) {
        Err(e) => Err(e),
        Ok(ps) => Ok({ name: name, index: index, parts: ps }),
      },
    },
  }
}
