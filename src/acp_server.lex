...
fn agent_info_json(info :: AcpAgentInfo) -> Str {
  jv.stringify(JObj([
    ("name",        JStr(info.name)),
    ("description", JStr(info.description)),
    ("version",     JStr(info.version)),
    ("url",         JStr(info.url)),
    ("capabilities", JObj([
      ("input",  JList([JStr("text")])),
      ("output", JList([JStr("text")])),
    ])),
  ]))
}
...
fn extract_first_user_text(j :: jv.Json) -> Str {
  match jv.get_field(j, "input") {
    None            => "",
    Some(JList(ms)) =>
      list.fold(ms, "", fn (acc :: Str, m :: jv.Json) -> Str {
        if str.is_empty(acc) {
          match jv.get_field(m, "content") {
            Some(JList(parts)) =>
              list.fold(parts, acc, fn (a :: Str, p :: jv.Json) -> Str {
                if str.is_empty(a) {
...
fn sse_started_frame(run_id :: Str) -> Str {
  sse_frame("run.started",
    jv.stringify(JObj([("run_id", JStr(run_id)), ("status", JStr("running"))])))
}

fn sse_text_chunk_frame(text :: Str) -> Str {
  sse_frame("message",
    jv.stringify(JObj([("type", JStr("text")), ("text", JStr(text))])))
}
