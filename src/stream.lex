...
fn decode_payloads(lines :: List[Str]) -> List[jv.Json] {
  let collected := list.fold(lines, { items: [], done: false }, fn (acc :: { items :: List[jv.Json], done :: Bool }, line :: Str) -> { items :: List[jv.Json], done :: Bool } {
    if acc.done {
      acc
    } else {
      decode_line(acc, line)
    }
  })
  collected.items
}

fn decode_line(acc :: { items :: List[jv.Json], done :: Bool }, line :: Str) -> { items :: List[jv.Json], done :: Bool } {
...
fn decode_statuses(lines :: List[Str]) -> List[tk.StatusUpdate] {
  ...
  list.fold(payloads, [], fn (acc :: List[tk.StatusUpdate], j :: jv.Json) -> List[tk.StatusUpdate] {
