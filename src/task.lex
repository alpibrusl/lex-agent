...
fn state_of(s :: Str) -> Option[TaskState] {
  if s == "submitted"      { Some(TSSubmitted) }
  else { if s == "working" { Some(TSWorking) }
  else { if s == "input-required" { Some(TSInputRequired) }
  else { if s == "completed" { Some(TSCompleted) }
  else { if s == "canceled"  { Some(TSCanceled) }
  else { if s == "failed"    { Some(TSFailed) }
  else { None }}}}}}
}
...
type TaskError =
    InvalidTransition({ from :: Str, to :: Str })
  | UnknownState(Str)

fn task_error_message(e :: TaskError) -> Str {
  match e {
    InvalidTransition(t) => str.concat("invalid transition ",
      str.concat(t.from, str.concat(" → ", t.to))),
    UnknownState(s)      => str.concat("unknown state: ", s),
  }
}

fn legal_transition(from :: TaskState, to :: TaskState) -> Bool {
  if is_terminal(from) { false }
  else {
    match from {
      ...
      TSWorking => match to {
        ...
        TSCanceled      => true,
        TSFailed        => true,
        TSWorking       => true,    # idempotent progress ticks
        _               => false,
      },
...
type StatusUpdate = {
  ...
  state :: TaskState,
  message :: Option[msg.Message],
  final :: Bool,                # true when state is terminal
}
