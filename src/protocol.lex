...
fn err_parse()            -> Int { 0 - 32700 }
fn err_invalid_request()  -> Int { 0 - 32600 }
fn err_method_not_found() -> Int { 0 - 32601 }
fn err_invalid_params()   -> Int { 0 - 32602 }
fn err_internal()         -> Int { 0 - 32603 }
fn err_task_not_found()   -> Int { 0 - 32001 }
fn err_not_cancelable()   -> Int { 0 - 32002 }
fn err_unsupported_op()   -> Int { 0 - 32004 }
fn err_spec_denied()      -> Int { 0 - 32099 }
...
type Response =
    ResOk(RpcId, jv.Json)
  | ResErr(RpcId, RpcError)

# ---- Methods ------------------------------------------------------

fn method_tasks_send()           -> Str { "tasks/send" }
fn method_tasks_get()             -> Str { "tasks/get" }
fn method_tasks_cancel()          -> Str { "tasks/cancel" }
fn method_tasks_send_subscribe()  -> Str { "tasks/sendSubscribe" }
...
fn parse_request_json(j :: jv.Json) -> Result[Request, RpcError] {
  let ver_ok := match jv.get_field(j, "jsonrpc") {
    Some(v) => match jv.as_str(v) { Some(s) => s == "2.0", None => false },
    None    => false,
  }
  if not ver_ok {
...
fn response_to_str(r :: Response) -> Str { jv.stringify(response_to_json(r)) }

# Convenience builders ----------------------------------------------

fn ok(id :: RpcId, result :: jv.Json) -> Response { ResOk(id, result) }
