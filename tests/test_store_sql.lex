    ...
    Ok(s1) => {
      ...
      match open_at(path) {
        ...
        Ok(s2) => match store.get(s2, "t_persist") {
          ...
          Some(found) => if found.id == "t_persist" {
            Ok(())
          } else {
...
fn check_full(t :: tk.Task) -> Result[Unit, Str] {
  match t.state {
    TSCompleted => if list.len(t.artifacts) == 1 {
      if list.len(t.history) >= 1 {
        Ok(())
      } else {
        Err("history not restored")
      }
    } else {
...
fn suite() -> [sql, fs_write, crypto, random, concurrent] List[Result[Unit, Str]] {
  [task_survives_reopen(), full_task_round_trips(), put_then_get_round_trips(), get_unknown_is_none(), cancel_transitions_and_persists(), cancel_unknown_returns_not_found(), cancel_terminal_rejected()]
}

fn run_all_count() -> [sql, fs_write, crypto, random, concurrent] Int {
...
fn run_all() -> [sql, fs_write, crypto, random, concurrent] Unit {
  if run_all_count() == 0 {
    ()
  } else {
    let __boom := 1 / 0
