...
fn echo_cap() -> cap.Capability {
  cap.inbound("echo", "test echo skill",
    { title: "Args", description: "", fields: [] })
}
...
fn test_agent(log :: trail.Log) -> srv.AgentDef {
  let base := srv.make_agent_def(
    card.make("test", "test agent", "0.0.1", "http://localhost",
      [echo_cap()]),
    [{ capability: echo_cap(), handle: echo_handler }])
...
fn has_kind(log :: trail.Log, kind :: Str) -> [sql] Bool {
  match trail.range(log, 0, 9999999999999) {
    Err(_) => false,
    Ok(events) =>
      list.fold(events, false, fn (acc :: Bool, e) -> Bool {
        acc or e.kind == kind
      }),
  }
}
...
fn test_dispatch_emits_task_received() -> [sql, fs_write, time, io, crypto, random, fs_read, net, concurrent] Result[Unit, Str] {
  match trail.open_memory() {
    ...
    Ok(log) => {
      ...
      if has_kind(log, "a2a.task.received") { Ok(()) }
      else { Err("missing a2a.task.received event") }
    },
  }
}
...
fn test_dispatch_emits_state_change() -> [sql, fs_write, time, io, crypto, random, fs_read, net, concurrent] Result[Unit, Str] {
  match trail.open_memory() {
    ...
    Ok(log) => {
      let agent := test_agent(log)
      let _resp := srv.dispatch_request(agent, good_envelope())
      if has_kind(log, "a2a.task.state_change") { Ok(()) }
      else { Err("missing a2a.task.state_change event") }
    },
  }
}
...
fn test_dispatch_emits_message_sent() -> [sql, fs_write, time, io, crypto, random, fs_read, net, concurrent] Result[Unit, Str] {
  match trail.open_memory() {
    ...
    Ok(log) => {
      let agent := test_agent(log)
      let _resp := srv.dispatch_request(agent, good_envelope())
      if has_kind(log, "a2a.message.sent") { Ok(()) }
      else { Err("missing a2a.message.sent event") }
    },
  }
}
...
fn suite() -> [sql, fs_write, time, io, crypto, random, fs_read, net, concurrent] List[Result[Unit, Str]] {
  [
    test_dispatch_emits_task_received(),
    test_dispatch_emits_state_change(),
    test_dispatch_emits_message_sent(),
    test_dispatch_no_trail_is_fine(),
  ]
}
