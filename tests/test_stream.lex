...
fn encode_includes_data_prefix() -> Result[Unit, Str] {
  ...
  if str.starts_with(s, "data: ") and str.ends_with(s, "\n\n") {
    Ok(())
  } else { Err(str.concat("bad encode: ", s)) }
}

fn decode_basic_event() -> Result[Unit, Str] {
  let lines := [
    "data: {\"id\":\"t_1\",\"state\":\"working\",\"final\":false}",
    "",
    "data: {\"id\":\"t_1\",\"state\":\"completed\",\"final\":true}",
    "",
  ]
  let xs := ssem.decode_statuses(lines)
  if list.len(xs) == 2 { Ok(()) } else {
...
fn decode_skips_done_marker() -> Result[Unit, Str] {
  ...
  let xs := ssem.decode_statuses(lines)
  if list.len(xs) == 1 { Ok(()) } else { Err("DONE should terminate") }
}
...
fn decode_ignores_comments() -> Result[Unit, Str] {
  ...
  let xs := ssem.decode_statuses(lines)
  if list.len(xs) == 1 { Ok(()) } else { Err("comment broke parse") }
}
...
fn decode_drops_malformed_payloads() -> Result[Unit, Str] {
  ...
  let xs := ssem.decode_statuses(lines)
  if list.len(xs) == 1 { Ok(()) } else { Err("malformed crashed parse") }
}

fn suite() -> List[Result[Unit, Str]] {
  [
    encode_includes_data_prefix(),
    decode_basic_event(),
    decode_skips_done_marker(),
    decode_ignores_comments(),
    decode_drops_malformed_payloads(),
  ]
}
