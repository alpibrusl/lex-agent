...
fn render_ok_includes_result() -> Result[Unit, Str] {
  ...
  if str.contains(s, "\"jsonrpc\":\"2.0\"")
    and str.contains(s, "\"id\":42")
    and str.contains(s, "\"result\":\"done\"")
  { Ok(()) } else { Err(str.concat("bad rendering: ", s)) }
}

fn render_error_includes_code() -> Result[Unit, Str] {
  let r := proto.fail(IdStr("x"), proto.err_spec_denied(), "blocked")
  let s := proto.response_to_str(r)
  if str.contains(s, "\"error\":") and str.contains(s, "\"code\":-32099")
    and str.contains(s, "blocked")
  { Ok(()) } else { Err(str.concat("bad error rendering: ", s)) }
}
...
fn parses_string_id() -> Result[Unit, Str] {
  ...
  match proto.parse_request(body) {
    Err(e)  => Err(e.message),
    Ok(req) => match req.id {
      IdStr(s) => if s == "abc" { Ok(()) } else { Err(str.concat("got: ", s)) },
...
fn render_null_id() -> Result[Unit, Str] {
  ...
  let s := proto.response_to_str(r)
  if str.contains(s, "\"id\":null") { Ok(()) } else { Err(s) }
}

fn suite() -> List[Result[Unit, Str]] {
  [
    parse_valid_request(),
    rejects_wrong_jsonrpc_version(),
    rejects_missing_method(),
    render_ok_includes_result(),
    render_error_includes_code(),
    parses_string_id(),
    render_null_id(),
  ]
}
