  ...
  match trail.open_memory() {
    ...
    Ok(log) => {
      ...
      match trail.head(log) {
        ...
        Some(e) => if e.kind == "a2a.task.sent" { Ok(()) }
                   else { Err(str.concat("expected a2a.task.sent, got: ", e.kind)) },
      }
    },
  }
}
  ...
  match trail.open_memory() {
    ...
    Ok(log) => {
      ...
      match trail.head(log) {
        ...
        Some(e) => if e.kind == "a2a.message.sent" { Ok(()) }
                   else { Err(str.concat("expected a2a.message.sent, got: ", e.kind)) },
      }
    },
  }
}
...
fn suite() -> [sql, fs_write, time, net, crypto, random] List[Result[Unit, Str]] {
  [
    test_send_task_traced_emits_task_sent(),
    test_subscribe_traced_emits_message_sent(),
  ]
}
