...
fn echo_skill() -> cap.Capability {
  cap.inbound("echo", "echo input back",
    {
      title: "EchoArgs",
      description: "",
...
fn demo_card() -> card.AgentCard {
  card.make("ping-pong", "demo agent", "0.1.0", "http://localhost:4040",
    [echo_skill()])
}

fn name_in_json() -> Result[Unit, Str] {
  let s := card.card_to_str(demo_card())
  if str.contains(s, "\"name\":\"ping-pong\"") { Ok(()) } else { Err(s) }
}

fn capabilities_in_json() -> Result[Unit, Str] {
  let s := card.card_to_str(demo_card())
  if str.contains(s, "\"streaming\":true") { Ok(()) } else { Err("streaming missing") }
}

fn skill_listed() -> Result[Unit, Str] {
  let s := card.card_to_str(demo_card())
  if str.contains(s, "\"name\":\"echo\"")
    and str.contains(s, "\"inputSchema\":")
  { Ok(()) } else { Err(str.concat("skill missing: ", s)) }
}

fn skill_input_schema_includes_constraint() -> Result[Unit, Str] {
  let s := card.card_to_str(demo_card())
  if str.contains(s, "\"minLength\":1") { Ok(()) } else {
