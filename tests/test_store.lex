...
fn put_then_get_round_trips() -> [sql, concurrent] Result[Unit, Str] {
  ...
  match store.get(addr, "t_round") {
    ...
    Some(found)  => if found.id == "t_round" { Ok(()) } else {
...
fn cancel_terminal_rejected() -> [sql, concurrent] Result[Unit, Str] {
  ...
  let _ok := store.cancel(addr, "t_term")    # first cancel succeeds
  match store.cancel(addr, "t_term") {       # second cancel must fail
    Err(_) => Ok(()),
...
fn suite() -> [sql, concurrent] List[Result[Unit, Str]] {
  [
    ...
    get_unknown_is_none(),
    put_replaces_existing(),
    cancel_transitions_state(),
