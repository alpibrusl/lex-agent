...
fn peer_url() -> Str { "http://127.0.0.1:4041" }
...
fn show_send_task(text :: Str, opts :: client.SendOpts) -> [net, crypto, random] Str {
  ...
  match client.send_task(peer_url(), m, opts) {
    Err(rpc) => str.concat("send-task-denied: code=",
      str.concat(int.to_str(rpc.code),
        str.concat(" message=", rpc.message))),
...
fn format_task_result(j :: jv.Json) -> Str {
  ...
  str.concat("send-task: state=",
    str.concat(state,
      str.concat(" text=", reply_text)))
}
...
fn demo() -> [net, crypto, random] Str {
  let line1 := show_card()
  let line2 := show_send_task("hello",
    { task_id: "t_lex_1", context_id: "ctx_lex_1", skill: "echo" })
  let line3 := show_send_task("",
    { task_id: "t_lex_2", context_id: "ctx_lex_1", skill: "echo" })
