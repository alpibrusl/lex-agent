  ...
  match store.open_store(SqliteStore(db_path)) {
    ...
    Ok(s)  => {
      let base := srv.make_agent_def(
        card.make("ping-pong-sqlite", "demo", "0.3.0",
          "http://localhost:4040", [echo_capability()]),
...
fn send_envelope() -> Str {
  str.concat(
    ...
    str.concat("\"id\":\"t_demo\",\"contextId\":\"ctx_demo\",",
      "\"message\":{\"kind\":\"message\",\"messageId\":\"msg_in\",\"role\":\"user\",\"parts\":[{\"type\":\"text\",\"text\":\"hello\"}]}}}"))
}

fn get_envelope(id :: Str) -> Str {
  str.concat(
    "{\"jsonrpc\":\"2.0\",\"id\":2,\"method\":\"tasks/get\",\"params\":{",
    str.concat("\"id\":\"", str.concat(id, "\"}}")))
}
...
fn demo() -> [io, time, crypto, random, sql, fs_read, fs_write, net, concurrent] Str {
  ...
  match make_agent(db_path) {
    ...
    Ok(agent_a) => {
      ...
      # Simulated restart: a brand-new agent + store handle at the
      # same path, with no memory of agent_a's send.
      match make_agent(db_path) {
